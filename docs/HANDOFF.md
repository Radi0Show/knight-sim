...
**55 suites, 1 failing.** The failure is `verify-fullfight` on the SECOND token (`verify37`) — a real, newly-found divergence, deliberately left visible rather than parked; `verify21j` inside the same suite is one-to-one. The whole-fight diff — the point of the project — passes; see §3 for what "one-to-one" precisely claims.
...
| whole-fight diff vs the oracle | **`fullfight-verify21j` is ONE-TO-ONE** — all 12,011 comparable frames exact across all 176 columns, byte-identical oracle across four replays. Premise (tightened 2026-08-22): equip arrays pinned and **FOUR** replay feeds, not six — `--shuffle --bolts --grazes --shards`. `--swords` and `--roar` were RETIRED after dropping each, and both together, still diffed one-to-one. Four micro-tolerances, each carrying its measured whole-fight envelope in its comment (POSITION 0.05px, ANGLE 0.02deg, SCALE 5e-5, SOUL 0.01px — one root, the proprietary libm). **`fullfight-verify37` (a SECOND token, seed 37) is the new front: first divergence f577.** A tracking-sword `obj_tracking_sword_slash_extra_graze` band pays its +7 TP one frame late (game real f577, sim f578). The band's geometry and timing were fitted from 2,586 logged `place_meeting` probes on token 21 (`oracle_bandcheck.csv`) plus its f508 pre-move/pre-decrement receipts; token 37 is the first input to disagree with that fit, on one of two bands (spawned real f521 and f573 — see the `.bands.csv` sidecar). This is the highest-value open lead in the project: it is exactly the fitted-to-one-sample risk the tolerances and models carry, caught by the cheapest possible test. |
