...
| whole-fight diff vs the oracle | **row-exact through frame 808, diverges at 809** — turns 1-2 and the third menu are exact; damage ties round half-even; unpaid graze bands persist across turns. The front is Flurry's first slash orientation: its roll sits at an unattributed offset in the anchored stream, so the recording `fullfight-slashlog` (running/complete in traces/) logs every splitslash's flags+offsets and the sim replays them (`--slashes`, wired end to end). Run `/tmp/finish-slashlog.sh` after the recording to collect+diff. NOTE the whole Flurry stream is offset, so the teeth's live rolls will diverge next — either solve the launch pad k with the slash log's bits as solver input (mirror the Stars 4-draw pad in fight.js case 1) or extend the log/replay to teeth params. |
