...
**53/54 suites green.** The one failure is `verify-fullfight`, which is the
...
| whole-fight diff vs the oracle | **row-exact through frame 11808 against `fullfight-verify21j`** — turns 1-22 complete: all of phases 1-3 BOTH laps, the phase-4 gate, the charge-up, and ROARING to within its final barrage. Canonical premise: equip arrays pinned; replay feeds `--shuffle --bolts --swords --grazes --shards` (graze rows gate on their recorded inv AND active — the strike-vs-graze event order inside one collision phase rides the row). Tolerances, each with a measured whole-fight envelope in its comment: bullet POSITION 0.05px, ANGLE 0.02deg, SCALE 5e-5, SOUL 0.01px — all one root, the runner's proprietary atan2/trig last bits reaching clamped headings and the ROARING pull. Attribution tools: `KNIGHT_DRAW_TRACE=a-b`, `KNIGHT_HIT_DEBUG=1`, `KNIGHT_GRAZE_DEBUG=1`, `KNIGHT_TALK_DEBUG=1`, `KNIGHT_SHARD_DEBUG=1`, `KNIGHT_TT_DEBUG=a-b` (now prints view+inv), `KNIGHT_TUNNEL_DEBUG=1`, `KNIGHT_RS_DEBUG=a-b`. **Front: f11809 — ROARING's final barrage.** At the f11809 hit a homing starchild's b15_y forks by 0.1px and compounds: the conceded one-ULP soul residue (the pull's atan2 chain) feeds the child's DISCRETE turn decision (scr_rotatetowards sign at near-dead-ahead), and no tolerance absorbs a flipped decision. Downstream: a star cull at f11837, a catch at f11846, the final bar one bolt off at f12010. THE SETTLE IS A RECORDING, not code: extend oracle_fullfight.csx with a roar sidecar (frame, tempdir, fake_x, fake_y, player_suck, intensity, and each starchild's target_angle at its turn frames), re-record, and replay tempdir/target_angle like the other feeds — the same shape of concession as --grazes. |
