...
| whole-fight diff vs the oracle | **row-exact through frame 1396 against `fullfight-verify21h`** — turns 1-3, all menus, turn 4's delivery and its first 26 corridor spawns. Canonical premise: equip arrays pinned (DEFAULT_GEAR), graze pairings replayed (`--grazes`, the runner's pair enumeration at hit frames is unsolved). Attribution tools: `KNIGHT_DRAW_TRACE=a-b` (per-draw call sites), `KNIGHT_HIT_DEBUG=1`, `KNIGHT_GRAZE_DEBUG=1`. Front: a corridor-boundary waitset choose at ~f1397 (small stream offset after the tunnel's first hits); `fullfight-verify21i` records the manager's cycle state + per-frame `global.turntimer` to settle it and the per-turn clock-arming rule (practice.js, the `ac === 2 ? tl : tl - 1` note). |
