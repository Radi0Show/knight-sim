...
| whole-fight diff vs the oracle | **row-exact through frame 897 against `fullfight-verify21e`** (turns 1-3 through split-2's teeth). The ±1-draw offset that stalled f891 is SOLVED: the recording's party fought bare (the harness pinned battle stats but not `global.chararmor*`, so scr_damage's mantle brunt choose never ran) — the equip arrays are now pinned in the patch and verify21e is the canonical recording. Attribution tool: `KNIGHT_DRAW_TRACE=a-b` on fullfight-trace.mjs prints every draw with its call site. OPEN at the front: two COLLISION KNIFE-EDGES (f898 graze tick on a tooth's death frame; f947 a hit the sim lands one frame late) — see the collision-model note below. The `fullfight-verify21f` recording (hitlog instrumented) answers both; if it exists, read `traces/fullfight-verify21f.hitlog.csv` first. |
| collision model | `masksOverlapPrecise` (2nd gen, 48/48 t4 points) is NOT the runner's algorithm at knife-edges. Measured by two boot-probes (knight-research `oracle_toothmeet.csx`, `oracle_fracsweep.csx` → `traces/toothmeet*.csv`, `fracsweep.csv`): place_meeting quantizes B's x by FLOOR and y by ROUND-HALF-EVEN, candidacy is integer cells in the raw-position band — but the fight's own f904/f947 hits MISS under those semantics, so the collision EVENT path differs from place_meeting. The hitlog names the bullet per hit; fit scripts: knight-research `tools/fit-mstar.mjs`, `tools/fit-toothmeet.mjs`. Do NOT touch masksOverlapPrecise without running verify-contact + the fracsweep/toothmeet fits together. |
