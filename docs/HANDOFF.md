...
| whole-fight diff vs the oracle | **`fullfight-verify21j` is ONE-TO-ONE** — all 12,011 comparable frames exact across 176 columns. Premise: equip arrays pinned and FOUR replay feeds (`--shuffle --bolts --grazes --shards`; `--swords` and `--roar` retired 2026-08-22 after dropping each, and both, still diffed one-to-one), plus four measured micro-tolerances (POSITION 0.05px, ANGLE 0.02deg, SCALE 5e-5, SOUL 0.01px). **`fullfight-verify37` — a SECOND token, seed 37 — is the generalization test, and it has moved f577 -> f1625 -> f2315 -> f3169** on four fixes, every one of which held verify21j one-to-one. **Front: f3169**, a one-frame bullet-count blip (sim 23, oracle 22) on a sword-tunnel sword whose `image_yscale` is crossing zero — the known proprietary-trig region (see SCALE_TOL), except here the residue feeds a DISCRETE death decision that no tolerance can absorb, the same shape as the f11809 case tolerances did cover. Slot columns shift when a bullet dies, so read that region with a per-instance log, not the positional slots. |
