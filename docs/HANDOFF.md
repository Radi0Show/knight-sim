...
| whole-fight diff vs the oracle | **`fullfight-verify21j` is ONE-TO-ONE** — all 12,011 comparable frames exact across 176 columns. Premise: equip arrays pinned, FOUR replay feeds (`--shuffle --bolts --grazes --shards`), four measured micro-tolerances (POSITION 0.05px, ANGLE 0.02deg, SCALE 5e-5, SOUL 0.01px). **`fullfight-verify37` (second token, seed 37) is the generalization front: f577 -> f1625 -> f2315 -> f3169 -> f3199 across six fixes**, every one of which kept verify21j one-to-one. **Front: f3199**, and it is now MEASURED rather than guessed. The tunnel sword's swept probe has a labelled dataset — a throwaway oracle variant logs every firing, `KNIGHT_SWEEP_ALL` logs every evaluation, `tools/fit-lineprobe.mjs` joins them: 8,126 evaluations, 603 firings, two tokens. The shipping model (a stepped WALK, adopted 2026-08-22) scores 8,124 with zero false positives; its only misses are probe37 f3041 and f3199, both segments clipping under a pixel outside a corner that the game still fires on. **Next: refine the SAMPLING, not the shape** — and read tools/fit-lineprobe.mjs's header first, because three separate measurement traps (2dp logging, exact join keys, and a non-cardinal `lengthdir`) each produced a confident wrong answer here. |
