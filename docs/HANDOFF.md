...
| whole-fight diff vs the oracle | **`fullfight-verify21j` is ONE-TO-ONE** — all 12,011 comparable frames exact across 176 columns. Premise: equip arrays pinned, FOUR replay feeds (`--shuffle --bolts --grazes --shards`), four measured micro-tolerances (POSITION 0.05px, ANGLE 0.02deg, SCALE 5e-5, SOUL 0.01px). **`fullfight-verify37` (second token, seed 37) is the generalization front: f577 -> f1625 -> f2315 -> f3169 -> f3199 on five fixes**, every one of which kept verify21j one-to-one. **Front: f3199** — the sword tunnel's swept probe misses a firing the game makes. NOT a mystery any more, it is MEASURED: the oracle can log the probe's own firings, and `tools/fit-lineprobe.mjs` scores models over 8,126 samples / 603 firings from both tokens. The shipping model gets 8,123 with ZERO false positives and three FALSE NEGATIVES (verify21j f1507, verify37 f3041, f3199), each a segment clipping 0.3-1.4px outside a corner that GameMaker fires on because it WALKS the line rather than clipping it. A stepped walk scores 8,124 (fp=0, fn=2) and fixes f3199 — and is STILL the wrong trade, which the raw count hides: **the two models' errors are not worth the same.** f1507, which the clip misses, has inv = 1.2 in the recording — positive, so `event_user(5)` does nothing and the sword's `destroyonhit = 0` means the firing is free. f1500, which the walk misses, resets inv to 3.2 — a real damage hit. Sampling the far endpoint (ceil the step count) recovers f1500 and still leaves f5549 over-firing on a vertical probe a hair outside the right edge. **Next: score candidates by what their errors COST — check each mismatch frame against the recording's inv — and settle the f5549 over-fire.** |
