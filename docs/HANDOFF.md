...
**All 54 suites green — verify-fullfight included.** The whole-fight diff — the point of the project — passes; see §3 for what "one-to-one" precisely claims.
...
| whole-fight diff vs the oracle | **ONE-TO-ONE. All 12,011 comparable frames of `fullfight-verify21j` are exact** — every turn of phases 1-3 (both laps of the phase-3 loop), the phase-4 gate, the charge-up, ROARING with its finale cut, and the fight-ending strike, across all 176 columns. `npm run verify`: 54/54. Canonical premise, stated in full: equip arrays pinned; replay feeds `--shuffle --bolts --swords --grazes --shards --roar` (each gives up exactly one underivable stream — the shuffle algorithm, the bolt schedule, the sword headings, the graze pairing order with each row's resolved inv+active, the d2 homing-delay chain, and ROARING's pull tempdir, the runner's own atan2); four micro-tolerances, each with its measured whole-fight envelope in its comment (bullet POSITION 0.05px, ANGLE 0.02deg, SCALE 5e-5, SOUL 0.01px — all one root, the proprietary libm's last bits). The recording is byte-identical across four independent replays (21j == 21m == 21o). Stale probe/older-generation recordings retired to traces/retired/. |
