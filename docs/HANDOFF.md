...
| whole-fight diff vs the oracle | **row-exact through frame 884, diverges at 885** (turns 1-2 + Flurry's first slash/split/teeth). OPEN: a few one-draw events per Flurry inter-slash window — slash inits at stream 9/554/2181/4244 (recording) vs 9/553/2180/4241 (sim). THE DECISIVE PROBE IS RECORDED: `oracle_rngprobe.csv` (save dir; also re-runnable — the DESTRUCTIVE probe block in oracle_fullfight.csx logs one consumed random(1) per frame, f800-960). Analysis recipe: frames before Flurry's re-anchor (~f807) mismatch by design (the tracking turn's stream is replay-covered, not solved); from the re-anchor on, compare the sim's per-frame probe draw (consume 1/frame after stepFrame — see the session's inline script) against the log. If all post-anchor frames mismatch by a constant, scan the fresh anchored stream (gmlCreate(21+2000)) for both sides' first few values to get their absolute positions — the offset localises the missing consumer to a frame, then the trace names the event on that frame. `state.gmlRng.draws` counts positions. Remove the probe block from the patch before the next CANONICAL recording. |
